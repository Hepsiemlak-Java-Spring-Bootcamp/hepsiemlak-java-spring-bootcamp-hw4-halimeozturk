...
public class AdvertController {
    ...
    @PostMapping("/jdbc")
    void jdbcSave(@RequestBody AdvertDTO advertDTO){
        advertService.jdbcSave(advertDTO);
    }

    @GetMapping("/jdbc")
    ResponseEntity<List<AdvertDTO>> getAllListJdbc(){
        return ResponseEntity.ok(advertService.getAllListJdbc());
    }

    @GetMapping("/jdbc/{id}")
    ResponseEntity<AdvertDTO> getUserByIdWithJdbc(@PathVariable Long id){
        return ResponseEntity.ok(advertService.getUserByIdWithJdbc(id));
    }
}
